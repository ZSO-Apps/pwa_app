...
function renderContentActions(actions) {
  ...
  return [
    ...
    '<button type="button" class="content-add-button" data-online-only="true" data-content-menu-toggle aria-expanded="false" aria-label="Inhalt hinzufügen">' + PLUS_ICON + '</button>',
    '<div class="content-actions-menu" data-content-menu hidden>',
    '<a data-online-only="true" href="' + esc(editorUrl(actions)) + '">Markdown erstellen</a>',
    '<a data-online-only="true" href="' + esc(formBuilderUrl(actions)) + '">Formular erstellen</a>',
    '<a data-online-only="true" href="' + esc(quizBuilderUrl(actions)) + '">Quiz hinzufügen</a>',
    '<button type="button" data-online-only="true" data-content-import="markdown" data-import-title="Markdown importieren" data-import-accept=".md,.markdown,.txt,text/markdown,text/plain">Markdown importieren</button>',
    '<button type="button" data-online-only="true" data-content-import="pdf" data-import-title="PDF importieren" data-import-accept=".pdf,application/pdf">PDF importieren</button>',
    '<button type="button" data-online-only="true" data-content-import="picture" data-import-title="Bild importieren" data-import-accept="image/png,image/jpeg,image/webp,image/gif,.png,.jpg,.jpeg,.webp,.gif">Bild importieren</button>',
    '<button type="button" data-online-only="true" data-content-link data-link-title="Webseite verlinken">Webseite verlinken</button>',
    '<button type="button" data-online-only="true" data-content-folder data-folder-title="Ordner erstellen">Ordner erstellen</button>',
    '</div>',
    '<dialog class="content-import-dialog" data-content-import-dialog>',
    '<form class="content-import-card" data-online-only-form data-content-import-form>',
    ...
    '<button type="submit" data-online-only="true" data-content-submit>Importieren</button>',
    '</div>',
    '</form>',
    '</dialog>',
    '</div>',
...
export function renderMarkdownEditorPage(req, kachel, { mode = 'new', rel = '', dir = '', backUrl = '', values = {}, error = '' } = {}) {
  ...
  const body = [
    ...
    '<h1>' + heading + '</h1>',
    error ? '<p class="err">' + esc(error) + '</p>' : '',
    '<form method="POST" action="' + esc(action) + '" class="genform wide markdown-editor-form" data-online-only-form data-markdown-editor-form>',
    ...
    '</div>',
    '<button type="submit" data-online-only="true">' + (editing ? 'Änderungen speichern' : 'Markdown speichern') + '</button>',
    '</form>',
    '</article>',
  ].join('');
  return layout(req, { title: heading, body, extraHeadBeforeStyles, extraHead });
}
...
export function renderFormBuilderPage(req, kachel, { dir = '', backUrl = '', values = {}, error = '' } = {}) {
  ...
  const body = [
    ...
    error ? '<p class="err">' + esc(error) + '</p>' : '',
    '<form method="POST" action="' + esc(action) + '" class="genform wide form-builder-form" data-online-only-form data-form-builder>',
    '<input type="hidden" name="dir" value="' + esc(dir) + '">',
    ...
    '<button type="button" class="secondary-button" data-form-builder-add>+ Feld hinzufügen</button>',
    '</section>',
    '<button type="submit" data-online-only="true">Formular speichern</button>',
    '</form>',
    '<p><a href="' + esc(backUrl || folderUrl(kachel.id, dir)) + '" class="back">← Zurück</a></p>',
    '</article>',
  ].join('');
